...
export default class LoginSession extends TypedEmitter<LoginSessionEvents> {
  ...
  constructor(
    ...
    if (
      Object.keys(options).filter((k) => mutuallyExclusiveOptions.includes(k))
        .length > 1
    ) {
      throw new Error(
        "Cannot specify more than one of localAddress, httpProxy, socksProxy, or agent at the same time"
      );
    }
  ...
  set loginTimeout(value: number) {
    if (this._pollingStartedTime) {
      throw new Error(
        "Setting loginTimeout after polling has already started is ineffective"
      );
    }
  ...
  set accessToken(token: string) {
    ...
    if (
      ...
      throw new Error(
        "Token is for a different account. To work with a different account, create a new LoginSession."
      );
    }
    ...
    if (this._refreshToken) {
      ...
      if (decodedRefreshToken.sub != decoded.sub) {
        throw new Error(
          "This access token belongs to a different account from the set refresh token."
        );
      }
    }
  ...
  set refreshToken(token: string) {
    ...
    if (!aud.includes("derive")) {
      throw new Error(
        "The provided token is an access token, not a refresh token"
      );
    }
    ...
    if (!aud.includes(requiredAudience)) {
      throw new Error(
        `Token platform type is different from the platform type of this LoginSession instance (required audience "${requiredAudience}" but got "${aud.join(
          ","
        )}"`
      );
    }
    ...
    if (this._accessToken) {
      ...
      if (decodedAccessToken.sub != decoded.sub) {
        throw new Error(
          "This refresh token belongs to a different account from the set access token."
        );
      }
    }
  ...
  get steamGuardMachineToken(): string {
    return this._steamGuardMachineToken;
  }
  ...
  async startWithCredentials(
    ...
    if (this._startSessionResponse) {
      throw new Error(
        "A session has already been started on this LoginSession object. Create a new LoginSession to start a new session."
      );
    }
    ...
    let encryptionResult = await this._handler.encryptPassword(
      details.accountName,
      details.password
    );

    this._startSessionResponse =
      await this._handler.startSessionWithCredentials({
        ...
        steamGuardMachineToken:
          details.steamGuardMachineToken || this.steamGuardMachineToken,
      });
  ...
  private async _processStartSessionResponse(): Promise<StartSessionResponse> {
    ...
    for (let i of this._startSessionResponse.allowedConfirmations) {
      switch (i.type) {
        ...
        case EAuthSessionGuardType.DeviceCode:
          let codeType =
            i.type == EAuthSessionGuardType.EmailCode ? "email" : "device";
          this.emit("debug", `${codeType} code required`);

          let authResult = await (codeType == "email"
    ...
    if (validActions.length == 0) {
      throw new Error(
        "Login requires action, but we can't tell what kind of action is required"
      );
    }
    ...
    if (
      (this._startSessionResponse as StartAuthSessionWithQrResponse)
        .challengeUrl
    ) {
      let startSessionResponse: StartAuthSessionWithQrResponse = this
        ._startSessionResponse as StartAuthSessionWithQrResponse;
  ...
  private async _doPoll() {
    ...
    let pollResponse;
    try {
      pollResponse = await this._handler.pollLoginStatus(
        this._startSessionResponse
      );
    ...
    this._startSessionResponse.clientId =
      pollResponse.newClientId || this._startSessionResponse.clientId;
    ...
    if (pollResponse.refreshToken) {
      ...
      if (
        !this.accessToken &&
        this._platformType == EAuthTokenPlatformType.MobileApp
      ) {
      ...
      this._pollTimer = setTimeout(
        () => this._doPoll(),
        this._startSessionResponse.pollInterval * 1000
      );
    }
  }
  ...
  private async _attemptEmailCodeAuth(): Promise<boolean> {
    ...
    if (
      ...
      this._startSessionResponse.allowedConfirmations.some(
        (c) => c.type == EAuthSessionGuardType.MachineToken
      )
    ) {
      let result = await this._handler.checkMachineAuthOrSendCodeEmail({
        machineAuthToken: this.steamGuardMachineToken,
        ...(this
          ._startSessionResponse as StartAuthSessionWithCredentialsResponse),
      });

      this.emit(
        "debug",
        `machine auth check response: ${EResult[result.result]}`
      );
  ...
  async submitSteamGuardCode(authCode: string): Promise<void> {
    ...
    let needsEmailCode = this._startSessionResponse.allowedConfirmations.some(
      (c) => c.type == EAuthSessionGuardType.EmailCode
    );
    let needsTotpCode = this._startSessionResponse.allowedConfirmations.some(
      (c) => c.type == EAuthSessionGuardType.DeviceCode
    );
  ...
  async getWebCookies(): Promise<string[]> {
    ...
    if (
      ...
      ].includes(this._platformType)
    ) {
      if (
        !this.accessToken ||
        Date.now() - this._accessTokenSetAt.getTime() > 1000 * 60 * 10
      ) {
      ...
      let cookieValue = encodeURIComponent(
        [this.steamID.getSteamID64(), this.accessToken].join("||")
      );
    ...
    if (
      !finalizeResponse.jsonBody ||
      !finalizeResponse.jsonBody.transfer_info
    ) {
    ...
    let transfers: Promise<string[]>[] =
      finalizeResponse.jsonBody.transfer_info.map(
        ({ url, params }) =>
          new Promise(async (resolve, reject) => {
            ...
            try {
              if (url === "https://store.steampowered.com/login/settoken") {
                result = await axios.postForm(
            ...
            if (
              ...
              result.headers["set-cookie"].length == 0
            ) {
              return reject(new Error("No Set-Cookie header in result"));
            }

            if (
              !result.headers["set-cookie"].some((c) =>
            ...
            resolve(
              result.headers["set-cookie"].map((cookie) =>
  ...
  async refreshAccessToken(): Promise<void> {
    ...
    this.accessToken = (
      await this._handler.generateAccessTokenForApp(this.refreshToken)
    ).accessToken;
  }
  ...
  async renewRefreshToken(): Promise<boolean> {
    ...
    let { accessToken, refreshToken } =
      await this._handler.generateAccessTokenForApp(this.refreshToken, true);
